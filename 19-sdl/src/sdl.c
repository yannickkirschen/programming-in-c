...
void check_collision(ball_struct *s1, ball_struct *s2) {
    ...
    if ((s1->x - s2->x) * (s1->x - s2->x) + (s1->y - s2->y) * (s1->y - s2->y) >
        4 * RADIUS * RADIUS)
        return;

    m21 = (s2->style + WEIGHT_OFFSET) /
          (s1->style + WEIGHT_OFFSET);  // mass ratio
    x21 = s2->x - s1->x;
    y21 = s2->y - s1->y;
    vx21 = s2->vx - s1->vx;
    vy21 = s2->vy - s1->vy;
    ...
    if (x21 == 0.)
        x21 = 1.0E-5;   // Cheat to avoid special treatment for x1==x2
    a = y21 / x21;
...
void paint_all(SDL_Renderer *renderer, SDL_Texture *ballTexture) {
    ...
    for (i = 0; i < no_balls; i++) {
        src.x = 0;
        src.y = balls[i].style * RADIUS * 2;
        dest.x = (int) balls[i].x - RADIUS;
        dest.y = (int) balls[i].y - RADIUS;
...
int escape_pressed() {
    ...
    if (keyevent.type == SDL_MOUSEMOTION) {    // If mouse motion, stir up balls
        for (i = 0; i < no_balls; i++) {
            if ((keyevent.motion.x - balls[i].x) *
                (keyevent.motion.x - balls[i].x)
                + (keyevent.motion.y - balls[i].y) *
                  (keyevent.motion.y - balls[i].y)
                < RADIUS * RADIUS) {
    ...
    return keyevent.type == SDL_QUIT || (keyevent.type == SDL_KEYDOWN &&
...
SDL_Texture *init_ball_textures(SDL_Renderer *renderer) {
    ...
    Uint32 pixels[RADIUS * 2 * RADIUS * 2 *
                  BALL_STYLES], c; // BALL_STYLE: Number of Balls, each with x_size=y_size=RADIUS*2

    ballTexture = SDL_CreateTexture(renderer, SDL_PIXELFORMAT_ARGB8888,
                                    SDL_TEXTUREACCESS_STATIC, 2 * RADIUS,
                                    2 * RADIUS * BALL_STYLES);
    ...
    for (bl = 0; bl < BALL_STYLES; bl++) {
        ...
        for (i = 0; i < RADIUS * 2 * RADIUS * 2; i++) {
            ...
            if (x * x + y * y <= 1.) {
                z = sqrt(1 - x * x - y * y);
                phong = ((z * 0.8) + (-y * 0.5) + (-x * 0.3));
                if (phong < 0) phong = 0;
                spec = phong * phong * phong * phong;
                for (j = 0; j < 3; j++) {
                    rgbi[j] = (int) (
                            (rgb[j] * 0.3 + rgb[j] * phong * 0.6 + spec * 0.5) *
                            255);  // Ambient+Diffuse+Specular
                    if (rgbi[j] > 255) rgbi[j] = 255;
                }
                z = 1 - x * x - y * y;
                rgbi[3] = z <= 2. / RADIUS ? (int) (255 * z * RADIUS / 2.)
                                           : 255; // Antialiasing
